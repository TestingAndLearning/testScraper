...
public class ND2EPS extends ND1Revenue {
	...
	public String getEPSPeriodValue(Document document, int index) throws InterruptedException {
		System.out.println(document.getElementsByClass("crDataTable").get(1).select("tbody > td.mainRow"));
		Element epsNode = document.getElementsByClass("crDataTable").get(1).select("tbody > td.mainRow").get(15).select("td.valueCell").get(index);
